public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < data.length; i++) System.out.printf("%d: (Binary) %s (Octal) %s (Hex) %s (Sai) %s\n", data[i], binary(data[i]), octal(data[i]), hexaDecimal(data[i]), saiSeries(data[i]));
    }
    ...
    //base 36 0-9, A-Z
    public static String saiSeries(int decimal) {
        int[] saiSeries = {0, 0, 0, 0, 0, 0, 0, 0, 0};
        if (decimal < 36) saiSeries[8] = decimal;
        if (decimal >= 36) {
            for (int i = saiSeries.length - 1; i >= 0; i--) {
                saiSeries[i] = decimal % 36;
                decimal /= 36;
            } }
        String build = converter(saiSeries);
        return build;
    }//base 36
