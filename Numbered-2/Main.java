public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < data.length; i++) System.out.printf("%d: (Binary) %s (Octal) %s (Hex) %s (Andrew) %s\n", data[i], binary(data[i]), octal(data[i]), hexaDecimal(data[i]), andrewSeries(data[i]));
    }
    ...
    //base 36 0-9, A-Z
    public static String andrewSeries(int decimal) {
        int[] andrewSeries = {0, 0, 0, 0, 0, 0, 0, 0, 0};
        if (decimal < 36) andrewSeries[8] = decimal;
        if (decimal >= 36) {
            for (int i = andrewSeries.length - 1; i >= 0; i--) {
                andrewSeries[i] = decimal % 36;
                decimal /= 36;
            } }
        String build = converter(andrewSeries);
        return build;
    }//base 36
